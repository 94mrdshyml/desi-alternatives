...
test.describe('Auth Pages E2E', () => {
  ...
  test('renders register page with name, email input, send code button, checkboxes, and sign in link', async ({ page }) => {
    await page.goto('/register');
    await expect(page).toHaveTitle(/Create Account|Get Started/i);
    await expect(page.locator('#name-input')).toBeVisible();
    await expect(page.locator('#email-input')).toBeVisible();
    await expect(page.locator('#terms-checkbox')).toBeVisible();
    await expect(page.locator('#newsletter-opt-in')).toBeVisible();
    await expect(page.locator('#send-code-btn')).toContainText('Send 6-Digit Code');
