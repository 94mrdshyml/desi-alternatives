...
const content = `Building and scaling modern software in 2026 requires engineering teams to be faster, more reliable, and resilient against cloud vendor lock-in.
...
SELECT '${createBlogPostToolId()}', '${postId}', id FROM desi_tools WHERE slug = 'signoz';

INSERT OR IGNORE INTO blog_post_tools (id, post_id, desi_tool_id)
SELECT '${createBlogPostToolId()}', '${postId}', id FROM desi_tools WHERE slug = 'appsmith';
`;

import { writeFileSync } from 'fs';
writeFileSync('drizzle/seed-blog.sql', seedSql);
console.log('Generated drizzle/seed-blog.sql successfully!');
